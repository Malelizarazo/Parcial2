...
export class ActividadService {
  ...
  async crearActividad(actividadDTO: CrearActividadDTO): Promise<Actividad> {
    const { titulo, cupoMaximo, fecha } = actividadDTO;
    const simbolos = /[^a-zA-Z0-9\sáéíóúÁÉÍÓÚñÑ]/;
    if (!titulo || titulo.length < 15 || simbolos.test(titulo)) {
      throw new BadRequestException('El título debe tener mínimo 15 caracteres y no contener símbolos');
    }
  ...
  async cambiarEstado(id: number, cambiarEstadoDTO: CambiarEstadoDTO): Promise<Actividad> {
    ...
    if (nuevoEstado === 1) {
      ...
      if (actividad.estudiantes.length < Math.ceil(actividad.cupoMaximo * 0.8)) {
        throw new BadRequestException('Debe tener al menos 80% del cupo lleno para cerrar');
      }
    } else if (nuevoEstado === 2) {
      // Finalizar: sin cupo disponible
      if (actividad.estudiantes.length < actividad.cupoMaximo) {
        throw new BadRequestException('Solo puede finalizarse si no hay cupo disponible');
      }
    }
